...
public class ProductServiceImpl extends ServiceImpl<ProductMapper,Product> implements ProductService {
    ...
    //引入feign客户端,需要在启动类添加配置注解@EnableFeignClients
    @Autowired
     ...
     *    2. 成功 继续根据类别id查询商品数据  [按照销售量倒序 7条]
     *    3. 结果封装即可
     * @param categoryName 类别名称
     * @return r
     * @Cacheable value为分区名，key为分区中的key值，cacheManager为配置类中定义的缓存管理方法
     */
    ...
    public R promo(String categoryName) {
        ...
        // 类别服务中 返回的r中的data是Category类型，但是经过feign调用，feign会返回{json}
        // product中会自动的把json转成 LinkedHashMap
        ...
        // 使用Page进行分页，两个参数为页数、每页的数据个数
        IPage<Product> page = new Page<>(1,7);

        //返回包装数据 内部有对应的商品集合,也有分页的参数 例如: 总条数 总页数等等
     ...
     * @return r
     * TODO: @Cacheable不写cacheManager默认调用添加了@Primary注解的管理器
     */
     ...
     * @return
     * 注意：在@Cacheable进行属性拼接时，从方法中取值时都要加#
     */
